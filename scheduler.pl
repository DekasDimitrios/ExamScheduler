...
schedule_errors(A,B,_C,E) :-
	...
	setof(Student,(attends(Student,_)),L),  % get all students
	schedule_errors_aux(A,B,L),       		% call the auxiliary predicate
...
schedule_errors_aux(A,B,Students) :-
	...
	findall(C,(attends(S,C)),Courses),  % get all courses that the stuent attends
